...
window.addEventListener('scroll', () => {
    ...
    if (currentScroll > activationDistance) {
        ...
        effectActive = false;
        header.classList.remove('hiddenN'); // Show header below activation distance
    }
    ...
    if (effectActive && Math.abs(currentScroll - lastThresholdCheck) > scrollThreshold) {
        if (currentScroll > lastScrollTop) {
            ...
            header.classList.add('hiddenN');
        } else {
            // Scroll Up: Show the header
            header.classList.remove('hiddenN');
        }
...
const getItemWidth = () => {
    const itemWidth = document.querySelector('.gallery-item').getBoundingClientRect().width;
    const gap = 20; // Constant gap between images
    return itemWidth + gap; // Return width of an item + the gap
};

scrollLeftBtn.addEventListener('click', () => {
    const itemWidth = getItemWidth(); // Get width of one image with gap
    scrollContainer.scrollBy({ left: -itemWidth, behavior: 'smooth' }); // Scroll by one item
});
...
function introduceSidebar() {
    ...
    sidebar.classList.add('active'); // Add the 'active' class to make it visible
  }
  
  function removeSidebar() {
    const sidebar = document.querySelector('.sidebar');
    sidebar.classList.remove('active'); // Remove the 'active' class to hide it
  }